 ...
 * 
 * 有分布式文件系统DistributedFileSystem和本地文件系统LocalFileSystem(本地连接的磁盘，测试)
 * <p>
 *
...
// FS_DEFAULT_NAME_KEY = "fs.defaultFS"; core-default.xml
...
// FS_DEFAULT_NAME_KEY 的默认值           
  ...
  // 文件系统及其统计信息，存IdentityHashMap
  private static final Map<Class<? extends FileSystem>, Statistics> 
    statisticsTable =
      new IdentityHashMap<Class<? extends FileSystem>, Statistics>();
  
//在IdentityHashMap中，当且仅当k1==k2时，才认为两个键相等；而在正常的Map实现中（比如HashMap），当且仅当两个键k1和K2满足:k1==null?k2==null：k1.equals(k2)时，两个键才相等。

  /**
   * The statistics for this file system. 文件系统的统计信息
   */
...
//Kerberos Ticket,这里的Ticket是指Ticket-Granting-Ticket(TGT),是您访问集群中服务的凭证。
    String ticketCachePath =
      conf.get(CommonConfigurationKeys.KERBEROS_TICKET_CACHE_PATH);
//getBestUGI：Find the most appropriate UserGroupInformation to use
 ...
 *     根据FS_DEFAULT_NAME_KEY取值，如果FS_DEFAULT_NAME_KEY不存在，返回DEFAULT_FS
 */
...
//FS_CLIENT_RESOLVE_REMOTE_SYMLINKS_KEY = "fs.client.resolve.remote.symlinks";
//FS_CLIENT_RESOLVE_REMOTE_SYMLINKS_DEFAULT = true
  ...
  /**
   * Canonicalize the given URI.规范化给定的uri
   * 
  ...
  protected URI canonicalizeUri(URI uri) {
  	//URI端口：getPort ； 文件系统端口：getDefaultPort
...
//static final URI NAME = URI.create("file:///");
  ...
  public static FileSystem get(URI uri, Configuration conf) throws IOException {
    ...
    //都是null，使用 DEFAULT_FS
    if (scheme == null && authority == null) {     // use default FS
      return get(conf);//Returns the configured filesystem implementation.
    }

    //scheme不是null，在scheme和默认scheme相同的情况下，使用 DEFAULT_FS
    if (scheme != null && authority == null) {     // no authority
      URI defaultUri = getDefaultUri(conf);
      if (scheme.equals(defaultUri.getScheme())    // if scheme matches default
          && defaultUri.getAuthority() != null) {  // & default has authority
        return get(defaultUri, conf);              // return default 调用自身，执行上一个if
      }
    }

...
public class DefaultSpeculator extends AbstractService implements
  ...
  private final Clock clock;  // Clock：告诉你时间的接口
...
//   如果任务没有运行，则返回NOT_RUNNING
  ...
  private long speculationValue(TaskId taskID, long now) {
    // 先由taskID取出所在的jobid，再由jobid取出job对象
    Job job = context.getJob(taskID.getJobId()); 
    // 由taskID取出task对象
    Task task = job.getTask(taskID);  
    ...
    // 如果这个任务不在mayHaveSpeculated集合里，即还没推断执行
    if (!mayHaveSpeculated.contains(taskID)) {
      // 为这个任务预计一个总的执行时间，
      // 如果返回的这个时间是Long.MAX_VALUE，
      // 就返回 ON_SCHEDULE 表示不应该考虑推测这个任务
    ...
    for (TaskAttempt taskAttempt : attempts.values()) {
      // 先判断是不是在运行中，如果是的话，直接进行下一个if
      // 如果不是在运行中，判断是不是正在启动，如果是正在启动，进行下一个if
      if (taskAttempt.getState() == TaskAttemptState.RUNNING
          || taskAttempt.getState() == TaskAttemptState.STARTING) {
        // 如果加上这次的任务尝试大于1，就返回已经在推测了
        if (++numberRunningAttempts > 1) {
          return ALREADY_SPECULATING;
        }
        // 返回一个这次的任务尝试的 TaskAttemptId
        runningTaskAttemptID = taskAttempt.getID();
        // 估计的这次的任务尝试的总运行时间，包括已经过去的时间。
        ...
        // 即便启动备份任务实例也无济于事，因为它的结束时间达不到节省作业总运行时长的作用。
...
// }
  ...
  private int maybeScheduleASpeculation(TaskType type) {
    ...
    for (ConcurrentMap.Entry<JobId, AtomicInteger> jobEntry : containerNeeds.entrySet()) {
      ...
      Job job = context.getJob(jobEntry.getKey()); //取出这个jobid
      ...
      int numberAllowedSpeculativeTasks
          = (int) Math.max(minimumAllowedSpeculativeTasks,
              proportionTotalTasksSpeculatable * tasks.size()); // 占比
  ...
  static class TaskAttemptHistoryStatistics {
    ...
    public boolean notHeartbeatedInAWhile(long now) {
      // 当前时间 距离 最近接收一次心跳的时间小于等于 最长等待时长阈值，
      // 返回false，表示这段时间有心跳
      if (now - lastHeartBeatTime <= MAX_WAITTING_TIME_FOR_HEARTBEAT) {
        return false;
      } else {
        // 如果超过了阈值，就用当前时间重置心跳时间，
        // 返回true，表示这段时间没有接收到心跳
...
参考：[https://blog.csdn.net/beliefer/article/details/51249119](https://blog.csdn.net/beliefer/article/details/51249119)